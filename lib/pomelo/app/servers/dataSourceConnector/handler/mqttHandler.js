...
Handler.prototype.publish = function (msg, session, next) {
    ...
    let action = parse.parseActionFromTopic(topic);
    let self = this;

    function publishOriginalData() {
        let originalData = parse.parseOriginalDataFromMqttData({topic, payload});
        self._dataService.publishOriginalData(originalData, traceContext, (err, isSuccess) => {
            if (err) {
    ...
    function configUpdateACK() {
        let updateResult = parse.parseConfigUpdateResultFromMqttData({topic, payload});
        self._dataService.configUpdateACK(updateResult, traceContext, (err, isSuccess) => {
            if (err) {
                logger.error(err.message, traceContext);
                next(err);
            }
            if (isSuccess) {
                logger.info(`${updateResult.s} config update ack success`, traceContext);
                next(null, {
                    errcode: 0,
                    errmsg: "ok"
                });
                ...
                logger.error(`${updateResult.s} config update ack fail`, traceContext);
                next(null, {
                    errcode: 400,
                    errmsg: "fail"
                });
    ...
    function kickSession() {
        self._app.get("sessionService").kickBySessionId(session.id, "close client", (err) => {
            if (err) {
                ...
                logger.warn(`kick invalid data source link`);
                next(null, {
                    errcode: 400,
                    errmsg: "fail"
                });
...
Handler.prototype.subscribe = function (msg, session, next) {
    ...
    let dataSourceID = parse.parseDataSourceIDFromTopic(topic);
    let action = parse.parseActionFromTopic(topic);
    let self = this;
    ...
    function* dataSourceLink() {
        ...
        let linkedMsg = {
            ...
            dup: false,
            topic,
            payload: JSON.stringify({v: {config: dataSourceJSON.config}})
        };
